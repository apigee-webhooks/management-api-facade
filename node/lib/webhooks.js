...
function forEachResource (req, body, resource, callback){ // find if the request matches the resource path expression
					...
					var pattern = new RegExp(resource.path);
					var matchRegExp = pattern.test(url_parts.pathname);
					if( matchRegExp && (resource.verb === req.method) ) { // match found based on URL and verb! send request to webhook
